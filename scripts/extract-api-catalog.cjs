...
Options:
  ...
  --modular, -m            Generate modular API catalogs in api/ (anti-monolith mode)
...
function getEndpointDomain(ep) {
  let clean = ep.path.replace(/^\/api\/(v\d+\/)?/, '').replace(/^\/(v\d+\/)?/, '').replace(/^\//, '');
  const segment = clean.split('/')[0];
  if (segment && segment !== ':id' && !segment.startsWith(':') && !segment.startsWith('{')) {
    return segment.toLowerCase();
  }
  const base = path.basename(ep.file, path.extname(ep.file)).toLowerCase();
  if (base && base !== 'index' && base !== 'routes' && base !== 'route' && base !== 'app') {
    return base;
  }
  return 'general';
}
...
if (isModular) {
  // Modular Generation: Group by workspace & domain
  const domainMap = new Map();
  for (const ep of deduped) {
    const domain = getEndpointDomain(ep);
    const domainKey = `${ep.workspace}__${domain}`;
    ...
    domainMap.get(domainKey).endpoints.push(ep);
  }

  // Generate individual domain files in docs/wiki/api/
  if (updateWiki) {
    const apiDir = path.join(wikiDirectory, 'api');
    if (!fs.existsSync(apiDir)) {
      fs.mkdirSync(apiDir, { recursive: true });
    }

    for (const [key, item] of domainMap.entries()) {
      const fileName = `${key.replace(/[^a-zA-Z0-9_-]/g, '-')}.md`;
      const filePath = path.join(apiDir, fileName);
      const wsObj = workspaces.find(w => w.name === item.workspace) || { displayName: item.workspace };

      let domainMd = `# API Domain Catalog: \`${item.domain.toUpperCase()}\`\n\n`;
      domainMd += `- **Workspace**: \`${wsObj.displayName}\`\n`;
      domainMd += `- **Total Endpoints**: ${item.endpoints.length}\n`;
      domainMd += `- **Parent Document**: [03-api-and-contracts.md](../${path.basename(resolvedWikiPath)})\n\n`;
      domainMd += `## Endpoints Specification\n\n`;
      domainMd += `| Method | Endpoint Path | Auth Guard | Source File & Line | Description |\n`;
      domainMd += `| :--- | :--- | :--- | :--- | :--- |\n`;

      for (const ep of item.endpoints) {
        const methodBadge = `\`${ep.method}\``;
        const relToWiki = path.relative(apiDir, ep.fullPath).replace(/\\/g, '/');
        domainMd += `| ${methodBadge} | \`${ep.path}\` | ${ep.auth} | [\`${ep.file}#L${ep.line}\`](${relToWiki}#L${ep.line}) | ${ep.description} |\n`;
      }

      fs.writeFileSync(filePath, domainMd, 'utf8');
    }
    console.log(`📑 Generated ${domainMap.size} modular API domain catalog(s) in: ${apiDir}`);
  }

  // Build Executive Overview Table in 03-api-and-contracts.md
  for (const ws of workspaces) {
    const wsDomains = Array.from(domainMap.values()).filter(d => d.workspace === ws.name);
    const wsEndpointsCount = wsDomains.reduce((acc, d) => acc + d.endpoints.length, 0);

    md += `### Workspace: ${ws.displayName} (${wsEndpointsCount} endpoints)\n\n`;
    md += `| Domain / Service Group | Endpoints Count | Sample Routes | Detailed API Spec |\n`;
    md += `| :--- | :--- | :--- | :--- |\n`;

    if (wsDomains.length === 0) {
      md += `| _- | _No route definitions found_ | _- | _- |\n\n`;
    } else {
      for (const d of wsDomains) {
        const key = `${d.workspace}__${d.domain}`;
        const fileName = `${key.replace(/[^a-zA-Z0-9_-]/g, '-')}.md`;
        const sampleRoutes = d.endpoints.slice(0, 2).map(e => `\`${e.method} ${e.path}\``).join('<br>');
        md += `| **\`${d.domain}\`** | ${d.endpoints.length} | ${sampleRoutes} | [📄 View API Catalog](./api/${fileName}) |\n`;
      }
      md += `\n`;
    }
  }
} else {
  // Classic Monolithic Table Output
  for (const ws of workspaces) {
